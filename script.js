...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let cropper; // Variable para la instancia del recortador
  document
    ...
    .addEventListener("change", function (e) {
      ...
      if (file) {
        ...
        // Usamos una estructura que permite el recorte
        previewContainer.innerHTML = `
            <div style="width: 300px; height: 300px; margin: 0 auto;">
                <img id="imageToCrop" src="${fotoPerfilLocal}" style="max-width: 100%;">
            </div>
            <button type="button" id="cropBtn" style="margin-top:10px; background:${colorPrincipal}; border:none; padding:10px; color:white; border-radius:5px; cursor:pointer;">
                Recortar y Aplicar
            </button>
        `;
          ...
          .addEventListener("click", function () {
            ...
            previewContainer.innerHTML = `
                <div style="width: 150px; height: 150px; border-radius: 50%;
                            border: 4px solid ${colorPrincipal}; overflow: hidden; margin: 0 auto; display: flex; align-items: center; justify-content: center;">
                    <img id="fotoPreview" src="${finalImageUrl}" style="width: 100%; height: 100%; object-fit: cover;">
                </div>
            `;

            // Subir la imagen recortada
            croppedCanvas.toBlob((blob) => {
              subirFotoABackground(blob);
            });
...
function reiniciarTodo() {
  ...
  document.querySelectorAll(".color-option").forEach((opt) => {
    opt.classList.toggle(
      "selected",
      opt.getAttribute("data-color") === "#3498db",
    );
  });
  ...
  document.querySelectorAll(".font-option").forEach((opt) => {
    opt.classList.toggle(
      "selected",
      opt.getAttribute("data-font") === "'Poppins', sans-serif",
    );
  });
...
function generarHTMLCV(
  formato = document.getElementById("PreviewFormatoCV").value,
) {
  switch (formato) {
    case "clasico":
                  ...
                  experiencia
                    ...
                    <div class="section-content">
                        ${experiencia
                          ...
                          .map(
                            (item) =>
                              `<div class="experience-item">${item.replace("</li>", "")}</div>`,
                          )
                          .join("")}
                    </div>
                </div>`
...
function obtenerEstudios() {
  ...
  const inputsSuperior = document
    .getElementById("EducacionSuperiorCampos")
...
async function guardarDatos(mostrarAlerta = true) {
  ...
  document
    .querySelectorAll("#cvForm input, #cvForm select")
    .forEach((campo) => {
      if (campo.id && campo.id !== "FotoPerfil")
        datos.campos[campo.id] = campo.value;
    });

  ["EducacionSuperiorCampos", "Habilidades", "Experiencia", "Cursos"].forEach(
    (id) => {
      datos.campos[id] = [];
      const inputs = document.getElementById(id).getElementsByTagName("input");
...
async function eliminarCV(id, event) {
  ...
  const codigoConfirmacion = prompt(
    "Ingrese el código de confirmación para eliminar este currículum:",
  );
  ...
  if (
    confirm(
      "¿Estás seguro que deseas eliminar este currículum? Esta acción no se puede deshacer.",
    )
  ) {
    try {
